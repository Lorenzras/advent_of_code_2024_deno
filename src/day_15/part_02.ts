...
function getMoveableBoxes(current: Point, stack: Point[], map: Map, dir: Dir) {
  const nextPosition = getNewPosition(current, dir);
  const nextCellValue = map[nextPosition.y][nextPosition.x];
  ...
  if (dir === "<" || dir === ">") {
    ...
    if (nextCellValue === "#") {
      ...
      return getMoveableBoxes(
        { x: nextPosition.x - 1, y: nextPosition.y },
        [...stack, { x: nextPosition.x - 1, y: nextPosition.y }],
        map,
        dir
      );
      // next one's a box, add it and keep checking
      ...
      return getMoveableBoxes(
        { x: nextPosition.x + 1, y: nextPosition.y },
        [...stack, { x: nextPosition.x, y: nextPosition.y }],
        map,
        dir
      );
    ...
    if (nextCellValue === "#") {
      ...
      const leftStack = getMoveableBoxes(
        { x: nextPosition.x - 1, y: nextPosition.y },
        [...stack, { x: nextPosition.x - 1, y: nextPosition.y }],
        map,
        dir
      ...
      const rightStack = getMoveableBoxes(
        { x: nextPosition.x + 1, y: nextPosition.y },
        [...stack, { x: nextPosition.x, y: nextPosition.y }],
        map,
        dir
